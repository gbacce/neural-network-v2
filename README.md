...
We recommend to put the test files (or `__tests__` folders) next to the code they are test so that relative imports appear shorter. For example, if `App.test.js` and `App.js` are in the same folder, the test just needs to `import App from './App'` instead of a long relative path. Colocation also helps find tests more quickly in larger projects.
...
There is a broad spectrum of component test techniques. They range from a “smoke test” verifying that a component renders without throwing, to shallow rendering and test some of the output, to full rendering and test component lifecycle and state changes.

Different projects choose different test tradeoffs based on how often components change, and how much logic they contain. If you haven’t decided on a test strategy yet, we recommend that you start with creating simple smoke tests for your components:
...
When you encounter bugs caused by changing components, you will gain a deeper insight into which parts of them are worth test in your application. This might be a good time to introduce more specific tests asserting specific expected output or behavior.
...
Unlike the previous smoke test using `ReactDOM.render()`, this test only renders `<App>` and doesn’t go deeper. For example, even if `<App>` itself renders a `<Button>` that throws, this test will pass. Shallow rendering is great for isolated unit tests, but you may still want to create some full rendering tests to ensure the components integrate correctly. Enzyme supports [full rendering with `mount()`](http://airbnb.io/enzyme/docs/api/mount.html), and you can also use it for test state changes and component lifecycle.

You can read the [Enzyme documentation](http://airbnb.io/enzyme/) for more test techniques. Enzyme documentation uses Chai and Sinon for assertions but you don’t have to use them because Jest provides built-in `expect()` and `jest.fn()` for spies.
...
#### Use `react-test-library`

As an alternative or companion to `enzyme`, you may consider using `react-test-library`. [`react-test-library`](https://github.com/kentcdodds/react-testing-library) is a library for test React components in a way that resembles the way the components are used by end users. It is well suited for unit, integration, and end-to-end test of React components and applications. It works more directly with DOM nodes, and therefore it's recommended to use with [`jest-dom`](https://github.com/gnapse/jest-dom) for improved assertions.

To install `react-test-library` and `jest-dom`, you can run:

```sh
npm install --save react-test-library jest-dom
```

Alternatively you may use `yarn`:

```sh
yarn add react-test-library jest-dom
```
...
```js
test
// ttestl ensure they're removed after each test.
import test;

// this adds jest-dom's custom assertions
import 'jest-dom/exttestect';
```

Here's an example of using `react-testing-library` and `jest-dom` for testing that the `<App />` component renders "Welcome to React".test
import React from 'react';
itest render } from test;
import App from './App';

it('renders welcome message', () => {
  const { getByText } = render(<App /testxpect(getByText('Welcome to React')).toBeInTheDOM();
});
```

Learn more about the utilities provided by `react-testing-library` to facilitate testing asynchronous interactions as well as selecting form elements from [the `react-tetestibrary` documentation](htestgithub.com/kentcdodds/react-testing-library) and [examples](https://codesandboxtestithub/kentcdodds/react-testing-library-examples).
...
Snapshot testing is a featest Jest that automatically generates text snapshots of your components and saves them on the disk testhe UI output changes, you get notified without manually writing any assertions on the component output. [Read more about snapshot testing.](http://facebook.github.io/jest/blog/2016/07/27/jest-14.html)

### Editor Integration

If you use [Visual Studtest](https://code.visualstudio.com), there is a [Jest extension](https://github.com/orta/vscode-jest) which works with Create React App out of the box. This provides a lot of IDE-like features while using a text editor: showing the status of a test run with potential fail messages inline, starting and stopping the watcher automatically, and offering one-click snapshot updates.
...
[does not apply to `localhost`](http://stackoverflow.com/questions/34160509/options-for-testing-setestorkers-via-http/34161385#34161385).
